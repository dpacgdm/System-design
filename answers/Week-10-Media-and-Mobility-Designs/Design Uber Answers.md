...
### NYE SF root cause chain

1. Deploy changed location TTL from 30s to 300s, keeping ghost drivers in the geo index.
2. Driver service still showed ONLINE because status and location freshness were split-brain.
3. Redis geo filled with stale entries and CPU reached 98%, causing GEORADIUS/query timeouts.
4. Matching saw wrong candidates or no candidates, so success rate dropped despite 12,000 ONLINE drivers.
5. Surge remained 3x because apparent supply collapsed in the affected cells.

### First 10 minutes mitigation

1. Rollback TTL to 30s and stop the bad deploy in all markets.
2. Pause matching against stale entries by enforcing last_update <= 30s in dispatch.
3. Rebuild SF geo index from the last 60 seconds of location Kafka events.
4. Temporarily reduce live-map update fan-out and routing depth to protect Redis.
5. Manually cap or reset surge in affected SF cells after validating real supply.
6. Communicate degraded matching and keep payments/cancellations safe.

### Permanent fixes

1. Make availability and location freshness a single truth for matching.
2. Use atomic update of status plus location or derive ONLINE from fresh location stream.
3. Set hard TTL bounds in code and config policy.
4. Load test event markets with stale-entry injection.
5. Alert on candidate_count=0 while online_driver_count is high.
...
1. Names the core correctness invariant for ride-hailing dispatch system: a driver can be assigned to at most one active trip and a rider is charged at most once for an intended completed trip
...
1. Treating caches, queues, or CDN as the source of truth when ride-hailing dispatch system needs durable reconciliation.
...
1. Verify dashboards expose user-impact SLOs for ride-hailing dispatch system.
...
11. Verify dashboards expose user-impact SLOs for ride-hailing dispatch system.
...
21. Verify dashboards expose user-impact SLOs for ride-hailing dispatch system.
...
31. Verify dashboards expose user-impact SLOs for ride-hailing dispatch system.
