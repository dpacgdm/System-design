...
Producer schema evolution broke consumers that treated events as full state; missing idempotency amplified duplicates.
...
- Config clue: `schema_compatibility: none`
- Config clue: `carries_full_shipping_state: true`
...
- `only revert producer without repair`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `replay DLQ at unlimited speed`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `send emails without idempotency`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `make analytics source of truth`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
