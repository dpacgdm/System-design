...
A hot Kafka partition is blocked by poison-message retries; consumer scaling cannot parallelize one partition in a group.
...
- Config clue: `producer_acks: 1`
- Config clue: `consumer_max_poll_interval_ms: 300000`
...
- `double consumers without fixing hot partition`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `increase partitions mid-incident`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `drop failed bid events`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `set producer acks to 0`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
