...
Saga split brain and missing external idempotency caused duplicate side effects and uncertain money movement.
...
- Config clue: `lease_fencing_token_required: false`
- Config clue: `step_timeout_ms: 1000`
...
- `restart all orchestrators without fencing`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `retry PSP with new keys`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `email completion before ledger balanced`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `compensate inventory twice`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
