...
- Config clue: `orders.require_outbox_same_transaction: false`
...
- `drop the replication slot`: risks losing recent committed state or reading stale data as authoritative.
- `publish from search results`: uses a derived view as truth, so it can miss or invent records during repair.
- `replay everything at unlimited speed`: moves backlog into downstream overload and can duplicate side effects faster than teams can audit.
- `make the new schema required without defaults`: reintroduces the semantic incompatibility and guarantees another consumer split-brain.
...
Source of truth: paid orders table, order_outbox rows, connector LSNs, and stable order event ids.
...
- database constraint/CI test for same-transaction outbox
- backward-compatible schema gates
- slot-retained WAL time-to-fill alerts
- repair publisher requiring deterministic operation ids
