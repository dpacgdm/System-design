...
A legacy path skipped the transactional outbox, and CDC/replay controls converted missing events into duplicates and WAL risk.
...
- Config clue: `require_outbox_row_in_same_txn: false`
- Config clue: `event_key: order_id`
...
- `manual publish without dedup`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `drop replication slot without recovery plan`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `trust Kafka over OLTP`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
- `restart connector with snapshot always`: widens blast radius, hides correctness risk, or converts recoverable lag into data loss/duplicates.
